...
// lanjut deploy webpack ke firebase
// jadi vuex ke firebase lgs bisa koq ( pake npm firebase itu)
// klo pake vuefire ke
...
export default {
  ...
  computed: {
    ...
    // }
    showDiff () {
      if (this.count - this.counttwo >= 15 || this.counttwo - this.count >= 15) {
        if (this.count > this.counttwo) {
          return 'Player One is AHEAD'
        } else {
          return 'Player Two is AHEAD'
        }
    ...
    showWinner () {
      if (this.count === 110 || this.counttwo === 110) {
        if (this.count > this.counttwo) {
          return 'Player One is THE WINNER'
        } else {
          return 'Player Two is THE WINNER'
